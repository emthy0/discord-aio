...
async function play(serverQueue) {
  ...
  const gid = serverQueue.gid
  await updateQueue(serverQueue.voiceChannel, currentQueue(gid)).catch((err) =>
    console.log(err),
  )
  if (!audioResource) {
